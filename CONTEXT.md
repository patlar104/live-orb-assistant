...
- Canonical deep-dive workflows: `AGENTS.md` and `.github/copilot-instructions.md`
- Release (top-level): `npm run lint`, `npm run build`, `npm run test:e2e`, `npm run preview`, `git tag -a vX.Y.Z -m "vX.Y.Z"`, `git push --tags`
- Perf profiling (top-level): `npm run dev` and DevTools Performance recording (10–20s)
- Shader debug (top-level): verify `piz_compressed.exr` loads and inspect shader uniforms
